...
const createSchema = z.object({
  ...
  dept:            z.enum(['AC', 'CO', 'ET', 'MI', 'MI1', 'MI2', 'RE1', 'RE2', 'RH'] as const).optional(),
  ...
  // The register also uses planning expressions ("S3 Juin 2025", "Réunion du groupe")
  correctionDeadlinePlannedText: z.string().max(200).optional(),
  correctionDeadlineActualText:  z.string().max(200).optional(),
  correctionProgress:        z.number().min(0).max(1).optional(),
  ...
  clientResponseRef:         z.string().max(200).optional(),
  isRisk:                    z.boolean().optional(),
  isOpportunity:             z.boolean().optional(),
  riskDesignation:           z.string().optional(),
  opportunityDesignation:    z.string().optional(),
  needsSecondCapa:           z.boolean().optional(),
  // FOR-MI-05 identity: N° Fiche / Mois / date the NC was actually detected
  ncFicheNum:                z.number().int().positive().optional(),
  ncMonth:                   z.string().max(20).optional(),
...
export async function POST(req: NextRequest) {
  ...
  const nc = await createNc({
    ...
    correctionDeadlinePlannedText: d.correctionDeadlinePlannedText,
    correctionDeadlineActualText:  d.correctionDeadlineActualText,
    correctionProgress:         d.correctionProgress,
    ...
    clientResponseRef:          d.clientResponseRef,
    isRisk:                     d.isRisk,
    isOpportunity:              d.isOpportunity,
    riskDesignation:            d.riskDesignation,
    opportunityDesignation:     d.opportunityDesignation,
    needsSecondCapa:            d.needsSecondCapa,
    ncFicheNum:                 d.ncFicheNum,
    ncMonth:                    d.ncMonth,
    detectedAt:                 d.detectedAt ? new Date(d.detectedAt) : undefined,
    assignedTo:                 d.assignedTo,

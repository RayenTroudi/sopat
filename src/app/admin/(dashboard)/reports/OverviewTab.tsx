...
// Thème « SOPAT Portfolio » : cartes vert foncé sur vert d'eau, texte blanc.
const BRAND_TEAL = '#429181'
...
function Kpi({ label, value, sub, color }: { label: string; value: string; sub?: string; color?: string }) {
  return (
    <div className="rounded-xl p-4" style={{ background: BRAND_DARK }}>
      <p className="text-xs uppercase tracking-wide" style={{ color: BRAND_SOFT }}>{label}</p>
      <p className="text-xl font-bold tabular-nums mt-1" style={{ color: color ?? '#FFFFFF' }}>{value}</p>
      {sub && <p className="text-xs mt-0.5" style={{ color: BRAND_SOFT, opacity: 0.85 }}>{sub}</p>}
    </div>
  )
}
...
function ChartCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-xl overflow-hidden border" style={{ borderColor: BRAND_DARK, background: 'var(--admin-surface)' }}>
      <div className="px-5 py-3.5" style={{ background: BRAND_DARK }}>
        <h3 className="text-sm font-semibold text-white">{title}</h3>
        <div className="h-px w-16 mt-1.5" style={{ background: 'rgba(255,255,255,0.6)' }} />
      </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <Kpi label="Projets actifs" value={String(projects.active)} sub="études · réalisation · entretien" color={BRAND_ALERT_AMBER} />
        <Kpi label="Projets terminés" value={String(projects.completed)} color={BRAND_LIGHT_GREEN} />
        <Kpi label="Projets annulés" value={String(projects.cancelled)} color={BRAND_SOFT} />
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <Kpi label="Encaissé" value={fmtTnd(money.collected)} sub="encaissements clients · cumul" color={BRAND_LIGHT_GREEN} />
        <Kpi
        ...
        />
        <Kpi label="Dépensé" value={fmtTnd(money.spent)} sub="bons d'achat + extra dépenses · cumul" color={BRAND_ALERT_RED} />
        <Kpi
          label="Marge brute"
          value={money.margin !== null ? fmtTnd(money.margin) : '—'}
          sub={money.marginPct !== null ? `${money.marginPct}% du contracté` : 'budget contracté requis'}
          color={money.margin !== null && money.margin < 0 ? BRAND_ALERT_RED : BRAND_LIGHT_GREEN}
        />
      </div>
        ...
        <ChartCard title={`Dépenses mensuelles ${year}`}>
          <ResponsiveContainer width="100%" height={240}>
            <LineChart data={spendData} margin={{ top: 8, right: 12, bottom: 0, left: 0 }}>
              ...
              <Line type="monotone" dataKey="total" name="Dépenses" stroke={BRAND_TEAL} strokeWidth={2} dot={{ r: 3 }} />
            </LineChart>
          </ResponsiveContainer>
        </ChartCard>
        ...
        <ChartCard title="Projets par type">
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={typeData} layout="vertical" margin={{ top: 8, right: 12, bottom: 0, left: 40 }}>
              ...
              <Tooltip />
              <Bar dataKey="count" name="Projets" fill={BRAND_TEAL} />
            </BarChart>
          </ResponsiveContainer>
        </ChartCard>
      </div>

...
import { getSupplyRegisterSummaries, SUPPLY_FORM_CODE } from '@/lib/db/supply'
import { formatMoney, formatPercent } from '@/lib/supply-calc'
...
export default async function SupplyTrackingPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const [projects, registers] = await Promise.all([
    getProjectsForSelect(),
    getSupplyRegisterSummaries(),
  ])
    ...
    <div className="p-6 space-y-8">
      <div>
        ...
        <p className="text-xs mt-0.5" style={{ color: 'var(--admin-text-muted)' }}>
          {SUPPLY_FORM_CODE} — devis validé, livraisons sur site et achats fournisseurs, par chantier
        </p>
      </div>

      {/* ── FOR-AC-10 : les registres eux-mêmes ── */}
      <section className="space-y-3">
        {registers.length === 0 ? (
          <div className="rounded-xl border p-12 text-center" style={CARD}>
            <p className="text-sm" style={{ color: 'var(--admin-text-muted)' }}>
              Aucun registre d&apos;approvisionnement pour l&apos;instant.
            </p>
            <p className="text-xs mt-1" style={{ color: 'var(--admin-text-muted)' }}>
              Un registre se crée depuis l&apos;onglet « Achats » d&apos;un projet.
            </p>
          </div>
        ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--admin-border)', background: 'var(--admin-bg)' }}>
                    {['Projet', 'Réf.', 'Client', 'Coût prévisionnel', 'Coût réel',
                      'Respect du coût', 'Respect de quantité', 'Dépenses HTVA', ''].map((h) => (
                      <th key={h} className="text-left px-4 py-2.5 text-[11px] font-medium whitespace-nowrap"
                ...
                <tbody>
                  {registers.map((r) => {
                    const t = r.totals
                    const overBudget = t !== null && t.costComplianceRate !== null && t.costComplianceRate > 1
                    return (
                      <tr key={r.id} style={{ borderTop: '1px solid var(--admin-border)' }}>
                        <td className="px-4 py-2.5 text-[13px] font-medium" style={{ color: 'var(--admin-text)' }}>
                          {r.projectName}
                        </td>
                        <td className="px-4 py-2.5 font-mono text-[11px]" style={{ color: 'var(--admin-text-muted)' }}>
                          {r.projectReference}
                        </td>
                        <td className="px-4 py-2.5 text-xs" style={{ color: 'var(--admin-text-muted)' }}>
                          {r.clientName}
                        </td>
                        <td className="px-4 py-2.5 text-[13px] tabular-nums" style={{ color: 'var(--admin-text)' }}>
                          {formatMoney(t?.plannedTotalHtva)}
                        </td>
                        <td className="px-4 py-2.5 text-[13px] tabular-nums" style={{ color: 'var(--admin-text)' }}>
                          {formatMoney(t?.actualTotalHtva)}
                        </td>
                        <td className="px-4 py-2.5 text-[13px] font-medium tabular-nums"
                          style={{ color: overBudget ? 'var(--admin-amber)' : 'var(--admin-text)' }}>
                          {formatPercent(t?.costComplianceRate, 0)}
                        </td>
                        <td className="px-4 py-2.5 text-[13px] tabular-nums" style={{ color: 'var(--admin-text)' }}>
                          {formatPercent(t?.quantityComplianceRate, 2)}
                        </td>
                        <td className="px-4 py-2.5 text-[13px] tabular-nums" style={{ color: 'var(--admin-text)' }}>
                          {formatMoney(t?.purchaseTotalHtva)}
                        </td>
                        <td className="px-4 py-2.5">
                          <Link href={`/admin/projects/${r.projectId}?tab=achats`}
      ...
      {/* ── Synthèse BC / bons, distincte du registre FOR-AC-10 ── */}
      <section className="space-y-4">
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: 'var(--admin-text-muted)' }}>
            Synthèse des pièces d&apos;achat enregistrées pour un projet — complète le registre ci-dessus.
          </p>
        </div>
        ...
        {!projectId && (
          <div className="rounded-xl border p-12 text-center" style={CARD}>
            <p className="text-sm" style={{ color: 'var(--admin-text-muted)' }}>
              Sélectionnez un projet pour afficher ses bons de commande et de livraison.
            </p>
          </div>
        )}
          ...
          <>
            ...
            <div className="rounded-xl border overflow-hidden" style={CARD}>
              <div className="px-4 py-3" style={{ borderBottom: '1px solid var(--admin-border)' }}>
                <h3 className="text-[14px] font-semibold" style={{ color: 'var(--admin-text)' }}>Bons de commande</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                  ...
                  <tbody>
                    {data.orders.map(({ order, supplierName }) => (
                      <tr key={order.id} className="even:bg-[var(--admin-bg)]/40" style={{ borderTop: '1px solid var(--admin-border)' }}>
                        ...
                        <td className="px-4 py-2.5 text-[13px] tabular-nums" style={{ color: 'var(--admin-text)' }}>{Number(order.quantityPurchased).toLocaleString('fr-FR')}</td>
                        <td className="px-4 py-2.5 text-[13px] tabular-nums" style={{ color: 'var(--admin-text)' }}>{Number(order.unitPricePaid).toLocaleString('fr-FR', { minimumFractionDigits: 3 })}</td>
                        <td className="px-4 py-2.5 text-[13px] font-medium tabular-nums" style={{ color: 'var(--admin-text)' }}>{Number(order.totalCost).toLocaleString('fr-FR', { minimumFractionDigits: 3 })}</td>
            ...
            <div className="rounded-xl border overflow-hidden" style={CARD}>
              <div className="px-4 py-3" style={{ borderBottom: '1px solid var(--admin-border)' }}>
                <h3 className="text-[14px] font-semibold" style={{ color: 'var(--admin-text)' }}>Bons de livraison &amp; retour</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>

...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!post) return { title: 'Article introuvable — SOPAT' }

  const title = post.yoast_head_json?.title ?? stripHtml(post.title.rendered) + ' — SOPAT'
...
export default async function PostPage({ params }: Props) {
  ...
  const imgAlt = media?.alt_text || stripHtml(post.title.rendered)
        ...
        >
          {imgSrc && (
            ...
            <img
              src={imgSrc}
              alt={imgAlt}
              style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 40%', opacity: 0.35, mixBlendMode: 'luminosity' }}
            />
          )}
          ...
          <div style={{ maxWidth: '860px', margin: '0 auto', padding: 'clamp(100px,14vw,160px) 24px clamp(40px,5vw,60px)', position: 'relative', zIndex: 1, width: '100%' }}>
            <p style={{ fontFamily: 'var(--font-inter), sans-serif', fontSize: '10px', color: 'rgba(245,240,232,0.4)', fontWeight: 300, marginBottom: '16px' }}>
        ...
        <article style={{ background: 'var(--ivory)', padding: 'clamp(48px,7vw,88px) 24px' }}>
          <div style={{ maxWidth: '760px', margin: '0 auto' }}>
            <div
              ...
              dangerouslySetInnerHTML={{ __html: sanitizeHtml(proxyContentImages(post.content.rendered)) }}
            />
